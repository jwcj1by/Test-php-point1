...
//all the rows
			for ($j = 0; $j < $cols; $j++) {
				//all the cols
				if ($data["open" . $i . "_" . $j] == 1) {
					$click_count++;
				}
//get the number of opennode
			}
		}
		if ($rows * $cols - $click_count == $num) {
			$checkflag = 1;
		}
...
//set nodes to open,if it is can open
...
//if game is clear or game is over ?>
    document.forms[0].clickvalue.value = value;
    document.forms[0].submit();
}
<?php if ($checkflag == 0 && $click_count > 0) {
	echo 'setTimeout("timerun()",1000);';
}
//time running ?>
<?php if ($checkflag == 1) {
	echo 'alert("恭喜，雷全部清掉了!php100.com");';
}
?>
<?php if ($checkflag == 2) {
	echo 'alert("太挫了，又被雷炸死了");';
}
?>
